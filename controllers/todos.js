...
module.exports = {
  getTodos: async (req, res) => {
    ...
    try {
      ...
      const teamItems = await Todo.find()
        .where("team")
        .equals(req.user.team)
        .where("userId")
        .ne(req.user._id);
      //selects all users that have the same team as the current user
      const teamMembers = await User.find().where("team").equals(req.user.team);

      teamItems.forEach((item) => {
        //Adds userName from User collection to team todo object array
